...
const mockDeleteUtilisationRequest =
  deleteUtilisationRequest as jest.MockedFunction<
    typeof deleteUtilisationRequest
  >;
const mockDeleteUtilisationRequestResponse =
  deleteUtilisationRequestResponse as jest.MockedFunction<
    typeof deleteUtilisationRequestResponse
  >;
...
describe("DeleteUtilisationResolver", () => {
  ...
  describe("Mutation.deleteUtilisation", () => {
    it("should successfully delete utilisation calculation", async () => {
      ...
      expect(mockDeleteUtilisationRequest).toHaveBeenCalledWith(
        mockInput,
        "user_123",
      );
      expect(mockDeleteUtilisationRequestResponse).toHaveBeenCalledWith(
        mockResult,
      );
    ...
    it("should handle authentication errors", async () => {
      ...
      await expect(
        DeleteUtilisationResolver.Mutation.deleteUtilisation(
          {},
          { input: mockInput },
          mockContext,
        ),
    ...
    it("should handle service errors", async () => {
      mockRequireAuth.mockResolvedValue(mockClerkUser);
      mockDeleteUtilisationRequest.mockRejectedValue(
        new Error("Calculation Not Found"),
      );
