...
describe("deleteUtilisationRequest", () => {
  ...
  it("should successfully delete calculation", async () => {
    ...
    mockPrisma.utilisationCalculation.findUnique.mockResolvedValue(
      mockCalculation,
    );
    ...
    expect(mockPrisma.utilisationCalculation.findUnique).toHaveBeenCalledWith({
      where: { id: "calc_123" },
    });
    expect(mockPrisma.utilisationCalculation.delete).toHaveBeenCalledWith({
      where: { id: "calc_123" },
    });
  ...
  it("should throw error when user not found", async () => {
    ...
    await expect(
      deleteUtilisationRequest(mockInput, "user_123"),
