...
describe('calculateUtilisationRequest', () => {
  ...
  it('should handle perfect utilisation calculation', async () => {
    const perfectInput = { ...mockInput, billableHours: 160, targetUtilisation: 100 }
    ...
    mockPrisma.user.findUnique.mockResolvedValue(mockUser)
    mockPrisma.utilisationCalculation.create.mockResolvedValue(perfectCalculation)
  ...
  it('should handle decimal precision in calculation', async () => {
    const precisionInput = { ...mockInput, totalHours: 37.5, billableHours: 25 }
    ...
    mockPrisma.user.findUnique.mockResolvedValue(mockUser)
    mockPrisma.utilisationCalculation.create.mockResolvedValue(precisionCalculation)

    const result = await calculateUtilisationRequest(precisionInput, 'user_123')
