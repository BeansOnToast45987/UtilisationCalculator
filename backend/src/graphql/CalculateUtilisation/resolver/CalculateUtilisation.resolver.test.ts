...
const mockValidateCalculateUtilisationInput =
  validateCalculateUtilisationInput as jest.MockedFunction<
    typeof validateCalculateUtilisationInput
  >;
const mockCalculateUtilisationRequest =
  calculateUtilisationRequest as jest.MockedFunction<
    typeof calculateUtilisationRequest
  >;
const mockCalculateUtilisationRequestResponse =
  calculateUtilisationRequestResponse as jest.MockedFunction<
    typeof calculateUtilisationRequestResponse
  >;
...
describe("CalculateUtilisationResolver", () => {
  ...
  describe("Mutation.calculateUtilisation", () => {
    it("should successfully calculate utilisation", async () => {
      ...
      const result =
        await CalculateUtilisationResolver.Mutation.calculateUtilisation(
          {},
          { input: mockInput },
          mockContext,
        );

      expect(mockValidateCalculateUtilisationInput).toHaveBeenCalledWith(
        mockInput,
      );
      expect(mockRequireAuth).toHaveBeenCalledWith(mockContext);
      expect(mockCalculateUtilisationRequest).toHaveBeenCalledWith(
        mockInput,
        "user_123",
      );
      expect(mockCalculateUtilisationRequestResponse).toHaveBeenCalledWith(
        mockCalculation,
      );
