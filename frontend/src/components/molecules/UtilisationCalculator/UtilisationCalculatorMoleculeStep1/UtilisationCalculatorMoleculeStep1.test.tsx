  ...
  useTranslation: vi.fn(() => ({
    t: vi.fn((key: string) => {
      const translations: Record<string, string> = {
        ...
        "utilisationCalculator.information": "Utilisation rate is the percentage of total working hours that are billable. Industry best practice targets 70% - 80% to allow time for professional development, administrative tasks, and strategic work",
        "utilisationCalculator.subtitle": "Enter your hours for the period to calculate your utilisation rate",
        ...
        "utilisationCalculator.fields.totalHours.label": "Total Hours Worked This Week",
        "utilisationCalculator.fields.billableHours.label": "Billable Hours",
        "utilisationCalculator.fields.targetUtilisation.label": "Target Utilisation (%)",
      };
...
vi.mock("../../../atoms/index", () => ({
  ...
  CustomButton: vi.fn(({ label, onClick, disabled, ariaLabel, buttonType, ...props }) => (
    <button
  ...
  CustomTextField: vi.fn(({ label, value, onChange, error, helperText, ...props }) => (
  ...
  CustomDatePicker: vi.fn(({ label, value, onChange, error, helperText, ...props }) => (
...
describe("UtilisationCalculatorMoleculeStep1", () => {
  ...
  it("should render header component with title and tooltip when step1 is initialized", () => {
    ...
    const subtitle = screen.getByText("Enter your hours for the period to calculate your utilisation rate");
    expect(subtitle).toBeInTheDocument();

    const tooltip = screen.getByTestId("custom-tooltip");
    expect(tooltip).toBeInTheDocument();
    expect(tooltip).toHaveAttribute("data-text", "Utilisation rate is the percentage of total working hours that are billable. Industry best practice targets 70% - 80% to allow time for professional development, administrative tasks, and strategic work");
  });
  ...
  it("should render buttons component with calculate and clear functionality when actions are needed", () => {
    ...
    const calculateButton = screen.getByTestId("utilisation-calculator-molecule-step1-buttons-calculate-button");
    const clearButton = screen.getByTestId("utilisation-calculator-molecule-step1-buttons-clear-button");
