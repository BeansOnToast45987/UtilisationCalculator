...
const mockEnsureDateRangeIsValid =
  ensureDateRangeIsValid as jest.MockedFunction<typeof ensureDateRangeIsValid>;
const mockEnsureTotalHoursIsValid =
  ensureTotalHoursIsValid as jest.MockedFunction<
    typeof ensureTotalHoursIsValid
  >;
const mockEnsureBillableHoursIsValid =
  ensureBillableHoursIsValid as jest.MockedFunction<
    typeof ensureBillableHoursIsValid
  >;
const mockEnsureTargetUtilisationIsValid =
  ensureTargetUtilisationIsValid as jest.MockedFunction<
    typeof ensureTargetUtilisationIsValid
  >;
...
describe("validateCalculateUtilisationInput", () => {
  ...
  it("should call all validation functions with correct parameters", () => {
    ...
    expect(mockEnsureDateRangeIsValid).toHaveBeenCalledWith(
      "2025-01-01",
      "2025-01-31",
    );
  ...
  it("should propagate date range validation errors", () => {
    ...
    expect(() => validateCalculateUtilisationInput(mockInput)).toThrow(
  ...
  it("should propagate billable hours validation errors", () => {
    ...
    expect(() => validateCalculateUtilisationInput(mockInput)).toThrow(
      "Billable Hours Cannot Exceed Total Hours",
    );
  });
